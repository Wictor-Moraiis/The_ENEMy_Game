...
static void LoadBattleQuestionsInternal(BattleState* b, Dificuldade diff) {
    ...
    else if (b->enemyId == CHAR_VOZ_GRAVE) subject = 2; // Humanas
    else if (b->enemyId == CHAR_ENEMY_HUMANITIES) subject = 2; // Humanas

    int targetLevel = 1;
    if (diff == DIFICULDADE_MEDIA) targetLevel = 2;
    if (diff == DIFICULDADE_DIFICIL) targetLevel = 3;

    b->questionCount = GetRandomQuestions(b->questions, MAX_BATTLE_QUESTIONS, targetLevel, subject);

    if (b->questionCount < MAX_BATTLE_QUESTIONS) {
        int extra = GetRandomQuestions(
            &b->questions[b->questionCount],
            MAX_BATTLE_QUESTIONS - b->questionCount,
            1,
            subject
        );
...
bool StartBattle(CharacterID enemyId, Dificuldade diff) {
    ...
    const char* bgPath = "assets/graphics/livro_background.png";

    if (enemyId == CHAR_MATH_ENEMY) {
        bgPath = "assets/graphics/numeros_background.png";
    }
    else if (enemyId == CHAR_NATUREZA_BOSS) {
        bgPath = "assets/graphics/jungle_background.png";
    }
    else if (enemyId == CHAR_ENEMY_HUMANITIES) {
        bgPath = "assets/graphics/humanidades_background.png";
    }
    else if (enemyId == CHAR_MASCARA_ARTES) {
        bgPath = "assets/graphics/livro_background.png";
    }

    Texture2D bg = LoadTexture(bgPath);
